...
Usage:
    ...
    python wm_minter.py --price-check         # print live WM price and exit
...
# Multi-hop: WM → AFFECTION → WPLS (PulseX V1 router for reliable path)
WPLS_ADDR      = "0xA1077a294dDE1B09bB078844df40758a5D0f9a27"
AFFECTION_ADDR = "0x24F0154C1dCe548AdF15da2098Fdd8B8A3B8151D"
# Use V1 router — V2 router (0x98bf93eb...) internally references V1 factory
PULSEX_V1_ROUTER = "0x165C3410fC91EF562C50559f7d2289fEbed552d9"
...
class WMMinter:
    ...
    Encapsulates all WM minting logic:
      ...
      - live price oracle (WM → AFFECTION → WPLS via PulseX V1 router)
    ...
    def __init__(self, rpc_url: str, tgsv5_address: str, private_key: str):
        ...
        self._router = self.w3.eth.contract(
            address=Web3.to_checksum_address(PULSEX_V1_ROUTER),
            abi=ROUTER_ABI,
        )
    ...
    def wm_price_pls(self) -> Decimal:
        """
        WM (MV) price in PLS via live PulseX V1 router query.

        Path: WM → AFFECTION → WPLS (multi-hop getAmountsOut).
        Uses V1 router (0x165C341...) — V2 router (0x98bf93eb...) internally
        references V1 factory and fails to find V2 pairs consistently.

        Checks WM_PRICE_PLS env override first (useful for testing / dry-runs).
        Falls back to 0.001 PLS/WM if the live query fails (e.g. no WM/AFF pair).
        """
        override = os.getenv("WM_PRICE_PLS")
        if override:
            log.debug("wm_price_pls: using env override %s PLS", override)
            return Decimal(override)

        path = [
            Web3.to_checksum_address(WM_CONTRACT),
            Web3.to_checksum_address(AFFECTION_ADDR),
            Web3.to_checksum_address(WPLS_ADDR),
        ]
        try:
            amounts = self._router.functions.getAmountsOut(10 ** 18, path).call()
            price = Decimal(amounts[-1]) / Decimal(10 ** 18)
            log.debug("wm_price_pls: live oracle %.8f PLS/WM", price)
            return price
        except Exception as exc:
            log.warning("wm_price_pls: oracle failed (%s) — fallback 0.001 PLS/WM", exc)
            return Decimal("0.001")

    def print_price_info(self) -> None:
        """Print WM price, gas price, and break-even batch size."""
        gas_price = self.gas_price_wei()
        wm_price  = self.wm_price_pls()
        print(f"  WM price:   {wm_price:.8f} PLS/WM  (live oracle)")
        print(f"  Gas price:  {gas_price / 1e9:.2f} Gwei  ({gas_price} wei)")
        for batch in (5, 10, 25, 50, 100):
            try:
                cost = self.mint_cost_pls(batch)
                value = wm_price * Decimal(batch)
                profit = value - cost
                note = "PROFITABLE" if profit > 0 else "not profitable"
                print(f"  batch={batch:3d}: cost={float(cost):.6f} PLS  value={float(value):.6f} PLS  "
                      f"profit={float(profit):+.6f} PLS  [{note}]")
            except Exception as exc:
                print(f"  batch={batch:3d}: estimate failed ({exc})")
...
def main() -> None:
    ...
    parser.add_argument(
        "--price-check", action="store_true",
        help="Print live WM price and break-even analysis, then exit (no TGSv5 needed)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="TGSv5 contract address (required for minting, not for --price-check)",
    )
    parser.add_argument(
        "--key", default=os.getenv("OPERATOR_PRIVATE_KEY",
                                   os.getenv("DYSNOMIA_PRIVATE_KEY", "")),
        help="Operator wallet private key (also reads DYSNOMIA_PRIVATE_KEY)",
    )
    args = parser.parse_args()

    # --price-check needs only RPC, not a deployed TGSv5 or full key
    if args.price_check:
        ...
        try:
            ...
            price_pls = amounts[-1] / 1e18
            print(f"\n  WM price (live): {price_pls:.8f} PLS/WM")
            print(f"  path: WM → AFFECTION → WPLS via PulseX V1")
        except Exception as exc:
            print(f"\n  Price oracle failed: {exc}")
            print("  Tip: set WM_PRICE_PLS env var to override.")
        return
    ...
    if args.count is None:
        ...
        tx = minter.run_manual(args.count, force=args.force)

...
*/
        ...
        public async Task<string> Deploy(OutputCallback Output, string file, params dynamic[] Args) {
            string diskfile = Path.Combine(RootFolder, file);
            ...
            if(!File.Exists(diskfile)) diskfile = Path.Combine(RootFolder, "scripts", file);
        ...
        public static (string ABI, string BIN) Compile(string file) {
            ...
            string diskfile = Path.Combine(_rootFolder, file);
            if(!File.Exists(diskfile)) _rootFolder = Path.Combine(_rootFolder, "solidity");
            diskfile = Path.Combine(_rootFolder, file);
            if(!File.Exists(diskfile)) throw new Exception("File Not Found: " + file);
            ...
            _p.Start();
            // Read stdout and stderr concurrently — prevents deadlock when solc
            // fills its stderr buffer (warnings) while we're waiting for stdout.
            Task<string> stdoutTask = _p.StandardOutput.ReadToEndAsync();
            Task<string> stderrTask = _p.StandardError.ReadToEndAsync();

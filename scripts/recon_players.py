#!/usr/bin/env python3
"""Player recon — find LAU, QING, YUE, and SHIO balances for any player on PulseChain.

Usage:
  python scripts/recon_players.py                                   # scan all known players
  python scripts/recon_players.py --address 0xccE8...               # scan specific player
  python scripts/recon_players.py --address 0xAAA... --address 0xBBB...  # scan multiple
"""
import argparse
from web3 import Web3
import json

# Known players for convenience (scan all when no --address given)
KNOWN_PLAYERS = {
    "enteh": "0xccE83CfF8B531EaDdcf11AB414C59DC046D1aAc7",
}
...
def scan_player(w3, player_addr):
    """Run full recon on a single player address."""
    PLAYER = Web3.to_checksum_address(player_addr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Player recon: LAU, QING, YUE, SHIO balances for any PulseChain player"
    )
    parser.add_argument("--address", type=str, action="append",
                        help="Player wallet address to scan (repeatable). "
                             "When omitted, scans all known players.")
    ...
    if args.address:
        ...
        addresses = list(KNOWN_PLAYERS.values())
        print(f"No --address given, scanning {len(addresses)} known player(s):")
        for label, addr in KNOWN_PLAYERS.items():
            print(f"  {label}: {addr}")

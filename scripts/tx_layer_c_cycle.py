#!/usr/bin/env python3
"""
tx_layer_c_cycle.py — PARADE → TLRz Treasury Cycle (Layer C)

Stage 3 of the FDIC→DFM→PARADE→TLRz cascade. Cycle uses 5 self-deployed ammo
(幹C01-C05, Parent=TARGET, Debenture=true) to pump TARGET into TLRz (Maria's
pre-existing V2 Federal token whose Parent is TARGET).

Phases:
  --phase approve    Static MAX approvals (WM→V2Minter if needed, PARADE→TLRz)
  --phase deploy     Deploy 5 幹C ammo (Parent=PARADE) + PARADE→ammo + ammo→TLRz
  --phase probe      1-unit sanity: TLRz.mint(1) + TLRz.Claim(幹C01, 1)
  --phase cycle      One 11-TX batch (TLRz.mint(5N) + 5×ammo.mint(N) + 5×Claim(N-K))
...
TARGET_PARENT = Web3.to_checksum_address("0xE37ACc54711562510FaFC45d8199Ee329ebBceDd")
TARGET        = Web3.to_checksum_address("0xC7145e1290B1d1221Aba5Ae48d4aCE17c6BE088F")
...
def _initial_state() -> dict:
    return {
        ...
        "target": {"symbol": "TLRz", "address": TARGET, "parent": TARGET_PARENT},
...
def do_approve(*, dry_run: bool, skip_confirm: bool) -> None:
    """Static MAX approvals: WM→V2Minter (if not already set), PARADE→TLRz."""
    preflight_gas()
    print(f"Phase: approve  dry_run={dry_run}")

    pairs = [
        ("WM → V2Minter", WM,  V2MINTER),
        ("PARADE → TLRz",  TARGET_PARENT, TARGET),
    ]
...
def do_deploy(*, dry_run: bool, skip_confirm: bool) -> None:
    """Deploy 5 幹C ammo (Parent=PARADE) + interleaved PARADE→ammo + ammo→TLRz approvals."""
    preflight_gas()
    state = load_state()
    ...
    if not pending:
        print("\nAll 5 Layer C ammo already deployed.")
        return
    ...
    for name, symbol in pending:
        ...
        predicted = _predict_ammo_address(name, symbol, AMMO_INITIAL_MINT, TARGET_PARENT)
        print(f"  predicted address: {predicted}")

        new_data = _encode_v2m_new(name, symbol, AMMO_INITIAL_MINT, TARGET_PARENT)
        receipt = send_tx(V2MINTER, new_data, label=f"V2Minter.New({symbol}, parent=PARADE)", dry_run=dry_run)
        ...
        if dry_run:
            ...
            if tt_parent(predicted) != TARGET_PARENT:
                raise RuntimeError(f"{symbol} Parent != PARADE — aborting")
            if v2m_treasury_owner(predicted) != JOEY:
                raise RuntimeError(f"{symbol} V2Minter.TreasuryTokens[ammo] != Joey — aborting")
            print(f"  verified: Debenture=true, Parent=PARADE, Owner=Joey")
        ...
        # PARADE → ammo approval
        send_tx(
            TARGET_PARENT,
            _encode_approve(predicted, MAX_UINT256),
            label=f"PARADE → {symbol} approve MAX",
            dry_run=dry_run,
        )
        ...
        send_tx(
            predicted,
            _encode_approve(TARGET, MAX_UINT256),
            label=f"{symbol} → TLRz approve MAX",
            dry_run=dry_run,
        )
...
def do_probe(*, dry_run: bool, skip_confirm: bool) -> None:
    """1-unit sanity check: verify TLRz accepts our ammo for Claim.

    TX 1: TLRz.mint(1) — seeds TLRz's PARADE vault with 1 PARADE
    TX 2: TLRz.Claim(幹C01, 1) — uses 1 幹C01 from initial deploy mint,
          returns 1 PARADE. If TLRz rejects our ammo (parent check / ownership
    ...
    if target.get("address", "").lower() != TARGET.lower():
        raise RuntimeError(f"State target mismatch — expected TARGET {TARGET}, got {target.get('address')}")

    # Static on-chain assertions before any TX
    print(f"Phase: probe  dry_run={dry_run}")
    print("\nStatic verification:")
    target_parent = tt_parent(TARGET)
    print(f"  TLRz.Parent(): {target_parent}  {'✓' if target_parent == TARGET_PARENT else '✗ expected PARADE'}")
    if target_parent != TARGET_PARENT:
        raise RuntimeError(f"TLRz.Parent != PARADE — aborting probe")
    ...
    print(f"  {ammo_sym}.Parent(): {par}  {'✓' if par == TARGET_PARENT else '✗ expected PARADE'}")
    if not deb:
        raise RuntimeError(f"{ammo_sym} Debenture=false — aborting probe")
    if par != TARGET_PARENT:
        raise RuntimeError(f"{ammo_sym} Parent != PARADE — aborting probe")

    # Allowances
    if erc20_allowance(TARGET_PARENT, JOEY, TARGET) < 10**18:
        raise RuntimeError("PARADE → TLRz allowance too low — run --phase approve")
    if erc20_allowance(ammo_addr, JOEY, TARGET) < 10**18:
        raise RuntimeError(f"{ammo_sym} → TLRz allowance too low — run --phase deploy")
    ...
    # PARADE inventory check
    dfm_bal = erc20_balance(TARGET_PARENT, JOEY)
    if dfm_bal < 10**18:
        raise RuntimeError(f"Joey has {dfm_bal/1e18} PARADE — need at least 1 for TLRz.mint(1)")

    confirm_or_abort(
        f"Run 2-TX probe? (TLRz.mint(1) + TLRz.Claim({ammo_sym}, 1) — consumes 1 {ammo_sym})",
        skip_confirm or dry_run,
    )

    # Pre-snapshot
    pre_parade = erc20_balance(TARGET, JOEY)
    pre_dfm    = erc20_balance(TARGET_PARENT,    JOEY)
    pre_ammo   = erc20_balance(ammo_addr, JOEY)

    # TX 1: TLRz.mint(1)
    send_tx(TARGET, _encode_tt_mint(10**18), label="TLRz.mint(1)", dry_run=dry_run)

    # TX 2: TLRz.Claim(幹C01, 1) — the critical test
    send_tx(
        TARGET,
        _encode_tt_claim(ammo_addr, 10**18),
        label=f"TLRz.Claim({ammo_sym}, 1)  [KEY TEST]",
        dry_run=dry_run,
    )
    ...
    time.sleep(2)
    post_parade = erc20_balance(TARGET, JOEY)
    post_dfm    = erc20_balance(TARGET_PARENT,    JOEY)
    ...
    print(f"\nPost-probe deltas:")
    print(f"  ΔTLRz:   {d_parade/1e18:+,.6f}   (expect +1)")
    print(f"  ΔPARADE: {d_dfm/1e18:+,.6f}      (expect 0: -1 mint + 1 claim)")
    print(f"  Δ{ammo_sym}: {d_ammo/1e18:+,.6f}  (expect -1)")

    if d_parade != 10**18:
        raise RuntimeError(f"TLRz delta wrong: {d_parade} vs +1e18")
    if d_dfm != 0:
        raise RuntimeError(f"PARADE delta wrong: {d_dfm} vs 0 (mint -1 + claim +1 should cancel)")
    if d_ammo != -10**18:
    ...
    save_state(state)
    print(f"\n✓ Probe successful. TLRz accepts our ammo.")
...
def do_cycle(*, n: int, hold_back: int = 0, dry_run: bool, skip_confirm: bool) -> None:
    """One batch cycle: TLRz.mint(5N) + ammo.mint(N)×5 + TLRz.Claim(ammo, N-K)×5."""
    if n <= 0:
    ...
    # PARADE balance check: 10N needed (5N for TLRz.mint + 5×N for ammo.mint)
    dfm_bal = erc20_balance(TARGET_PARENT, JOEY)
    if dfm_bal < 10 * N_wei:
        raise RuntimeError(f"PARADE balance {dfm_bal/1e18:,.0f} < 10×N {10*n:,}")

    # Allowances
    if erc20_allowance(TARGET_PARENT, JOEY, TARGET) < TOTAL:
        raise RuntimeError("PARADE → TLRz allowance too low — run --phase approve")
    for addr in ammo_addrs:
        if erc20_allowance(TARGET_PARENT, JOEY, addr) < N_wei:
            raise RuntimeError(f"PARADE → {addr} allowance too low — run --phase deploy")
        if not skip_claim and erc20_allowance(addr, JOEY, TARGET) < CLAIM_WEI:
            raise RuntimeError(f"{addr} → TLRz allowance too low — run --phase deploy")
    ...
    # Pre-snapshot
    pre = {
        "dfm_joey":    erc20_balance(TARGET_PARENT,    JOEY),
        "parade_joey": erc20_balance(TARGET, JOEY),
        "dfm_ammo":    [erc20_balance(TARGET_PARENT, a) for a in ammo_addrs],
        "joey_ammo":   [erc20_balance(a,   JOEY) for a in ammo_addrs],
    }
    print(f"\nPre-batch state:")
    print(f"  Joey PARADE: {pre['dfm_joey']/1e18:,.6f}")
    print(f"  Joey TLRz:   {pre['parade_joey']/1e18:,.6f}")
    for i, bal in enumerate(pre["dfm_ammo"]):
        print(f"  {ammo_entries[i]['symbol']} PARADE vault: {bal/1e18:,.6f}  |  Joey holds: {pre['joey_ammo'][i]/1e18:,.6f}")

    dfm_spend      = 5 * (n + hold_back)
    hold_back_note = "" if hold_back == 0 else f" + hold back {hold_back} of each ammo ({5*hold_back} total)"
    confirm_or_abort(
        f"Execute {tx_count}-TX batch? (spend {dfm_spend} PARADE, gain {5*n} TLRz{hold_back_note})",
        skip_confirm or dry_run,
    )

    # TX 1: TLRz.mint(5N)
    send_tx(TARGET, _encode_tt_mint(TOTAL), label=f"TLRz.mint({5*n})", dry_run=dry_run)
    ...
    if skip_claim:
        ...
        for entry, addr in zip(ammo_entries, ammo_addrs):
            send_tx(
                ...
                label=f"TLRz.Claim({entry['symbol']}, {claim_amt})",
                dry_run=dry_run,
            )
    ...
    print(f"\nPost-batch state:")
    print(f"  Joey PARADE: {post['dfm_joey']/1e18:,.6f}  (Δ {d_dfm/1e18:+,.6f})")
    print(f"  Joey TLRz:   {post['parade_joey']/1e18:,.6f}  (Δ {d_parade/1e18:+,.6f})")
    for i, (bal_now, bal_pre) in enumerate(zip(post["dfm_ammo"], pre["dfm_ammo"])):
        d    = bal_now - bal_pre
        held = post["joey_ammo"][i] - pre["joey_ammo"][i]
        print(f"  {ammo_entries[i]['symbol']} PARADE vault: {bal_now/1e18:,.6f}  (Δ {d/1e18:+,.6f})  |  Joey {ammo_entries[i]['symbol']} Δ {held/1e18:+,.6f}")
    ...
    if d_dfm != expected_dfm_delta:
        raise RuntimeError(f"PARADE delta mismatch: got {d_dfm}, expected {expected_dfm_delta}")
    if d_parade != expected_parade_delta:
        raise RuntimeError(f"TLRz delta mismatch: got {d_parade}, expected {expected_parade_delta}")
    for i, (bal_now, bal_pre) in enumerate(zip(post["dfm_ammo"], pre["dfm_ammo"])):
...
def do_verify() -> None:
    ...
    print(f"  PARADE: {erc20_balance(TARGET_PARENT, JOEY)/1e18:>22,.6f}")
    print(f"  TLRz:   {erc20_balance(TARGET,        JOEY)/1e18:>22,.6f}")
    ...
    dfm_to_parade = erc20_allowance(TARGET_PARENT, JOEY, TARGET)
    print(f"  WM  → V2Minter: {'MAX' if wm_to_v2m     >= MAX_UINT256 >> 1 else 'NOT SET'}")
    print(f"  PARADE → TLRz:  {'MAX' if dfm_to_parade >= MAX_UINT256 >> 1 else 'NOT SET'}")

    print(f"\n=== TARGET on-chain ===")
    print(f"  Parent:    {tt_parent(TARGET)}  {'✓ PARADE' if tt_parent(TARGET) == TARGET_PARENT else '✗'}")
    print(f"  Debenture: {tt_debenture(TARGET)} (not required for target)")
    print(f"  totalSupply: (not shown — large)")

    state = load_state()
    print(f"\n=== Layer C state ({STATE_FILE}) ===")
    ...
    if not state.get("ammo"):
        ...
        for ammo in state["ammo"]:
            addr = ammo["address"]
            bal_dfm_vault = erc20_balance(TARGET_PARENT, addr)
            bal_joey      = erc20_balance(addr, JOEY)
            dfm_to_a      = erc20_allowance(TARGET_PARENT, JOEY, addr)
            a_to_parade   = erc20_allowance(addr, JOEY, TARGET)
            deb_now       = tt_debenture(addr)
            print(f"  {ammo['symbol']:>6} @ {addr}")
            print(f"    PARADE locked:   {bal_dfm_vault/1e18:,.6f}")
            print(f"    Joey holds:      {bal_joey/1e18:,.6f}")
            print(f"    Debenture:       {deb_now}")
            print(f"    PARADE → ammo:   {'MAX' if dfm_to_a    >= MAX_UINT256 >> 1 else 'NOT SET'}")
            print(f"    ammo → TLRz:     {'MAX' if a_to_parade >= MAX_UINT256 >> 1 else 'NOT SET'}")


# ─── CLI ─────────────────────────────────────────────────────────────────
def main() -> int:
    parser = argparse.ArgumentParser(description="PARADE → TLRz treasury cycle (Layer C)")

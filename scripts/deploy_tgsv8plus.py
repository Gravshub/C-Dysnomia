#!/usr/bin/env python3
"""
Deploy TGSv8+ (TreasuryGameSharkv8PLUS) to PulseChain.

TGSv8+ = Companion contract for TGSv8. Handles mint→harvest→LP→burn pipeline.
Constructor takes 7 addresses: lau, payToken, wpls, routerV1, routerV2, factoryV1, factoryV2.

Usage:
  python scripts/deploy_tgsv8plus.py --dry-run        # compile only, print size + functions
  python scripts/deploy_tgsv8plus.py --update-env      # deploy + write TGSV8PLUS_ADDRESS to .env
  python scripts/deploy_tgsv8plus.py                   # deploy only (print address)
"""
import os
import sys
import subprocess
...
ABI_OUT       = REPO_ROOT / "scripts" / "Joystick" / "data" / "abis" / "tgsv8plus.json"
...
CONSTRUCTOR_ARGS = {
    "lau":       "0x66a08aa12da955eb63d7ac121a88b2b210a07b03",   # GIBS LAU
    "payToken":  "0x24F0154C1dCe548AdF15da2098Fdd8B8A3B8151D",   # AFFECTION
...
def compile_contract(source: str) -> tuple:
    """Compile TGSv8Plus.sol -> (abi, bytecode) with optimizer, NO viaIR."""
    print(f"\n[1/6] Compiling {CONTRACT_NAME} with solc {SOLC_VER} (optimize=200, no viaIR)...")
    input_json = {
        "language": "Solidity",
    ...
    contract = compiled["contracts"]["TGSv8Plus.sol"][CONTRACT_NAME]
    abi      = contract["abi"]
    bytecode = contract["evm"]["bytecode"]["object"]
    deployed = contract["evm"]["deployedBytecode"]["object"]
    deploy_len = len(deployed) // 2
    init_len = len(bytecode) // 2
    print(f"  Init bytecode: {init_len:,} bytes")
    print(f"  Deployed bytecode: {deploy_len:,} bytes ({deploy_len/1024:.1f} KB)  ABI entries: {len(abi)}")
    if deploy_len > 24576:
        print(f"  FATAL: Exceeds 24,576 byte EIP-170 limit!")
        sys.exit(1)
    else:
        print(f"  Under EIP-170 limit ({24576 - deploy_len:,} bytes to spare)")
    return abi, bytecode
...
def save_abi(abi: list):
    ...
    # Atomic write
    tmp = ABI_OUT.with_suffix(".tmp")
    tmp.write_text(json.dumps(abi, indent=2))
    os.replace(str(tmp), str(ABI_OUT))
...
def main():
    ...
    parser.add_argument("--update-env", action="store_true", help="Write TGSV8PLUS_ADDRESS to .env")
    ...
    if args.dry_run:
        fn_names = sorted([e['name'] for e in abi if e.get('type') == 'function'])
        ev_names = sorted([e['name'] for e in abi if e.get('type') == 'event'])
        print(f"\n[DRY-RUN] Compilation successful. No deploy.")
        print(f"  Functions ({len(fn_names)}):")
        for fn in fn_names:
            print(f"    - {fn}")
        print(f"  Events ({len(ev_names)}):")
        for ev in ev_names:
            print(f"    - {ev}")

        # Verify critical functions
        critical = ['silentMint', 'safeMint', 'harvestCycle', 'batchBurnLP',
                     'burnLPPercent', 'removeLiquidity', 'sellLau', 'sellLauMultiHop',
                     'execute', 'executeWithValue', 'batchExecute',
                     'deposit', 'withdraw', 'withdrawPLS', 'batchSweep']
        missing = [f for f in critical if f not in fn_names]
        if missing:
            print(f"\n  MISSING critical functions: {missing}")
        else:
            print(f"\n  All {len(critical)} critical functions present")
        return
    ...
    if not pkey:
        print("ERROR: DYSNOMIA_PRIVATE_KEY not set. Run: export DYSNOMIA_PRIVATE_KEY=0x...")
        sys.exit(1)
    ...
    gas_beats = gas_price / 1e9
    print(f"  Gas est: {gas_est:,}  Limit: {gas_limit:,}  Gas price: {gas_beats:.2f} Beats")
    ...
    actual_cost = used * gas_price / 1e18
    print(f"  Block: {blk:,}  Status: {status}  Gas used: {used:,}  Cost: {actual_cost:.4f} PLS")
    ...
    lau_addr       = contract.functions.lau().call()
    pay_addr       = contract.functions.payToken().call()
    lau_rem        = contract.functions.lauRemaining().call()

    print(f"  owner()           = {contract_owner}")
    print(f"  authorized(Joey)  = {is_authorized}")
    print(f"  lau()             = {lau_addr}")
    print(f"  payToken()        = {pay_addr}")
    print(f"  lauRemaining()    = {lau_rem}")

    assert contract_owner.lower() == JOEY_WALLET.lower(), "owner() mismatch!"
    assert is_authorized, "authorized(Joey) is False!"
    assert lau_addr.lower() == CONSTRUCTOR_ARGS["lau"].lower(), "lau() mismatch!"
    assert pay_addr.lower() == CONSTRUCTOR_ARGS["payToken"].lower(), "payToken() mismatch!"
    assert lau_rem > 0, "lauRemaining() is 0!"
    print("  All assertions passed")

    # ── Update .env ─────────────────────────────────────────────────────
    if args.update_env:
        print(f"\n[+] Writing TGSV8PLUS_ADDRESS to .env...")
        update_env_file("TGSV8PLUS_ADDRESS", address)
    ...
    print(f"  Block:    {blk:,}")
    print(f"  Gas:      {used:,} ({actual_cost:.4f} PLS)")
    ...
    if not args.update_env:
        print(f"\n  export TGSV8PLUS_ADDRESS={address}")
